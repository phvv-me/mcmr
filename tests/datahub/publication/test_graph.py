...
from mcmr_datahub import DataHubCodeGraph, DataHubOpenAPI, DataHubProvider, DataHubSettings
...
_GRAPH = RunGraph(
    ...
    jobs=[RuleJob(rule="ALL-DUPL0005", inputs=[_ANCHOR], primary=_ANCHOR)],
)
...
def timeline(rule: str, *, where: str = "", runs: int = 2) -> RuleTimeline:
    ...
    return RuleTimeline(
        ...
        events=[
            RunEvent(
                ...
                properties={"findings": str(item + 1), "path": where},
            )
...
def test_a_published_fact_dataset_states_its_columns_and_the_rows_this_run_read() -> None:
    ...
    assert receipts == [
        "chefe published 1 fact datasets as 5 entities owned by datahub in Codebases"
    ]
...
def test_everything_published_carries_the_owner_and_domain_a_reader_browses_by() -> None:
    ...
    assert aspect(dataset, "domains")["domains"] == [domain_urn("Repositories")]
    assert aspect(flow, "domains")["domains"] == [domain_urn("Repositories")]
    assert aspect(flow, "ownership")["owners"] == [
        {"owner": "urn:li:corpuser:pedro", "type": "TECHNICAL_OWNER"}
    ]
    assert aspect(entities["domain"][0], "domainProperties")["name"] == "Repositories"
    assert entities["domain"][0]["urn"] == domain_urn("Repositories")


def test_the_platform_card_carries_its_own_mark() -> None:
    """DataHub reads `logoUrl` as an image source, so the whole mark travels in the aspect."""
    entities, _ = emitted("publish")
    stated = aspect(entities["dataplatform"][0], "dataPlatformInfo")
    logo = stated["logoUrl"]

    assert isinstance(logo, str) and logo.startswith("data:image/svg+xml;base64,")
...
def test_a_rule_job_points_at_the_fact_table_its_verdicts_are_recorded_against() -> None:
    ...
    unread = _GRAPH.model_copy(update={"jobs": [RuleJob(rule="ALL-DUPL0005")]})
...
def test_the_ingestion_client_refuses_to_publish_before_it_is_opened() -> None:
    """A client with no connection pool cannot have written anything, so it says so."""
    openapi = DataHubOpenAPI(DataHubSettings(server="https://catalog.example"))

    with pytest.raises(RuntimeError, match="must be opened"):
        anyio.run(openapi.ingest, "dataset", [])

    anyio.run(openapi.__aexit__, None, None, None)
    assert openapi.client is None
...
def test_a_run_that_consumed_no_table_publishes_no_graph(tmp_path: Path) -> None:
    ...
    context = PublicationContext(
        repository=tmp_path,
        settings={"server": "https://catalog.example", "report_url": _REPORT},
...
def test_the_ingestion_client_refuses_to_read_before_it_is_opened() -> None:
    """A client with no connection pool cannot have read anything, so it says so."""
    openapi = DataHubOpenAPI(DataHubSettings(server="https://catalog.example"))

    with pytest.raises(RuntimeError, match="must be opened"):
        anyio.run(openapi.read, "datajob", [])
